...
请使用 root 用户登录你的服务器，然后执行以下命令直接安装：

wget -O install.sh [https://raw.githubusercontent.com/你的GitHub用户名/你的仓库名/main/install.sh](https://raw.githubusercontent.com/你的GitHub用户名/你的仓库名/main/install.sh) && bash install.sh


(注意：发布前请将上方命令中的 你的GitHub用户名 和 你的仓库名 替换为你真实的 GitHub 路径。)

🛠️ 连通性测试

安装完成后，可以在本地终端运行以下命令测试代理是否生效：
...
修改 ExecStart= 这一行中的 -u (用户名) 和 -P (密码) 参数。修改后重启服务即可：
...
如果你的服务器位于云服务商（如阿里云、腾讯云、AWS 等），请务必在云服务商的网页控制台 -> 安全组规则中，放行你设置的 TCP 端口，否则无法从外网连接。
